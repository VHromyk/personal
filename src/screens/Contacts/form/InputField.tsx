    ...
    <div className="flex flex-col gap-1 relative">
      <label htmlFor={id} className="font-medium">
        ...
        <input
          ...
          {...registerOptions}
        />
        {typeof error?.message === 'string' && <div className="absolute text-xs bottom-[-16px] text-error font-medium">{error.message}</div>}
    </div>
  )
}

export const TextAreaField = ({ id, label, placeholder, registerOptions, error }: IBaseField) => {
  return (
    <div className="flex flex-col gap-1 relative">
      <label htmlFor={id} className="font-medium">
        {label}
